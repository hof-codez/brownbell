...
interface RecapContent {
    ...
    topScorer: { teamName: string; playerName: string; playerPosition: string; points: number } | null;
    ...
    standingsTop3: { rank: number; teamName: string; combined: number; seasonTotal: number; bonusTotal: number }[];
}
...
// no normal app shell. Deliberately its own self-contained component
// (rather than reusing ShowdownTab's matchup cards) since this reads a
// single static, already-computed JSON blob rather than any of the
// app's live hooks - the whole point of a shared link is that it never
// changes after the fact, so there's nothing here to keep in sync with
// live data at all.
    ...
    return (
        <div className="min-h-screen bg-field px-4 py-8 text-chalk">
            <div className="mx-auto max-w-2xl">
                ...
                {content.matchupOfTheWeek && (
                    <Section title="Matchup of the Week">
                        <MatchupCard m={content.matchupOfTheWeek} highlight />
                    </Section>
                )}
                ...
                {content.biggestBlowout && (
                    <Section title="Biggest Blowout">
                        <MatchupCard m={content.biggestBlowout} note={`${content.biggestBlowout.margin.toFixed(1)} point margin`} />
                    </Section>
                )}
                    ...
                    <Section title="Top Scorer">
                        <div className="rounded-lg border border-panel-line bg-panel px-4 py-3">
                            <p className="font-body text-lg font-semibold text-chalk">
                                {content.topScorer.playerName} <span className="text-sm text-chalk-dim">({content.topScorer.playerPosition})</span>
                            </p>
                            <p className="font-mono text-2xl font-bold text-bell">{content.topScorer.points.toFixed(1)} pts</p>
                            <p className="font-body text-xs text-chalk-dim">{content.topScorer.teamName}</p>
                        </div>
                    </Section>
                )}

                {content.biggestUpset && (
                    <Section title="Biggest Upset">
                        <div className="rounded-lg border border-panel-line bg-panel px-4 py-3">
                            <p className="font-body text-sm text-chalk">
                                <span className="font-semibold text-chalk">{content.biggestUpset.winner}</span> beat{' '}
                                <span className="text-chalk-dim">{content.biggestUpset.loser}</span>
                                {' '}{content.biggestUpset.scoreWinner.toFixed(1)}-{content.biggestUpset.scoreLoser.toFixed(1)}
                            </p>
                            <p className="mt-1 font-mono text-xs text-chalk-dim">
                                Given only a {(content.biggestUpset.winnerProbability * 100).toFixed(0)}% chance beforehand
                            </p>
                        </div>
                    </Section>
                )}

                {content.leaguePredictions && (
                    <Section title="League Predictions">
                        <div className="mb-2 rounded-lg border border-panel-line bg-panel px-4 py-3 text-center">
                            <p className="font-mono text-2xl font-bold text-bell">
                                {content.leaguePredictions.record.correct}-{content.leaguePredictions.record.wrong}
                            </p>
                            <p className="font-body text-xs text-chalk-dim">the league&rsquo;s record calling this week&rsquo;s matchups</p>
                        </div>
                        <div className="flex flex-col gap-1.5">
                            {content.leaguePredictions.matchups.map((pm, i) => (
