...
class SupabaseDataLayer {
    ...
    async loadSubstitutions() {
        ...
        return data.map(row => ({
            ...
            active: row.active,
            // Raw value, needed by update-standings.js's subInfo capture
            // (see 034-weekly-scores-sub-info.sql) - autoGenerated below is
            // a derived boolean from this same column, kept for whatever
            // still reads it, but it was never a substitute for the raw
            // value itself, which nothing exposed until now.
            source: row.source,
    ...
    async getActiveSubstitutionsForWeek(week, awardType) {
        ...
        const { data, error } = await this.supabase
            .from('substitutions')
            .select('team_id, player_index, original_name, substitute_name, source, end_week, reason')
            .eq('award_type', awardType)
            .lte('start_week', week);
        ...
        // clean .lte()/.gte() chain against a nullable column. Also
        // excludes the same two reasons update-standings.js's own
        // current-week activeSub lookup excludes: the initial pick log
        // (every slot gets one the moment it locks, whether or not it's
        // ever subbed) and a revert back to the original after a
        // temporary sub - both leave a row matching this filter, but
        // neither means the player is actually a substitute. Confirmed as
        // the same class of bug: every player in the weekly recap was
        // showing as subbed with no original name, since this was
        // matching the initial-pick row for every slot regardless of
